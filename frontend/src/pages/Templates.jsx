...
const PRIORITY_COLOR = { low: 'text-emerald-600 bg-emerald-50', medium: 'text-amber-600 bg-amber-50', high: 'text-red-600 bg-red-50' }
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="font-display font-800 text-3xl text-ink">Templates</h1>
      ...
      {/* How it works banner */}
      <div className="bg-ink rounded-2xl p-4 mb-6 flex items-start gap-4">
        ...
        <div>
          <p className="text-paper font-display font-600 text-sm">How Templates Work</p>
          <p className="text-paper/40 text-xs mt-0.5 leading-relaxed">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {templates.map((tpl) => (
            <div key={tpl.id} className="card p-5 animate-slide-up hover:shadow-md transition-all">
              {/* Template header */}
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="font-display font-700 text-base">{tpl.name}</h3>
                  {tpl.description && (
                    <p className="text-xs text-ink/50 mt-0.5 line-clamp-2">{tpl.description}</p>
                  )}
                </div>
                <span className="badge bg-paper-soft text-ink/50 shrink-0 ml-2">
              ...
              <ul className="space-y-1.5 mb-4">
                {tpl.template_tasks
                  ...
                  .slice(0, 5)
                  .map((tt) => (
                    <li key={tt.id} className="flex items-center gap-2 text-xs text-ink/70">
                ...
                {tpl.template_tasks.length > 5 && (
                  <li className="text-[10px] text-ink/30 pl-3.5">
              ...
              {/* Actions */}
              <div className="flex items-center gap-2 pt-3 border-t border-paper-dim">
                <button
